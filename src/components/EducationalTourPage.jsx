    ...
    <div className="flex items-center gap-3">
      ...
      {item.viewerImage && (
        <button
          ...
          className="flex items-center justify-center w-[42px] h-[42px] rounded-xl bg-[#A1A1A1] hover:bg-[#8e8e8e] text-zinc-950 transition-all duration-200 shadow-md"
        >
          <ImageIcon size={18} />
        </button>
      )}
...
export default function EducationalTourPage({ onBack }) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white relative font-sans selection:bg-[#A1A1A1] selection:text-neutral-950 overflow-hidden">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative bg-[#111113] border border-zinc-800 rounded-2xl w-full max-w-md shadow-2xl z-10 flex flex-col overflow-hidden animate-in fade-in zoom-in-90 duration-300 ease-[cubic-bezier(0.16,1,0.3,1)]">
            <div className="p-5 border-b border-zinc-800/60 flex items-start justify-between gap-4 shrink-0 relative">
              <div className="pr-12">
                <span className="font-mono text-[10px] uppercase tracking-widest text-[#A1A1A1] mb-2 block font-bold">
            ...
            <div className="p-5 overflow-hidden relative bg-[#0f0f11]">
              <div className="flex items-start gap-3">
                <BookOpen size={20} className="text-[#A1A1A1] shrink-0 mt-0.5" />
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#0a0a0a] border border-zinc-800 rounded-2xl w-full max-w-xl shadow-2xl z-10 flex flex-col p-5 overflow-hidden animate-in fade-in zoom-in-90 duration-300 ease-[cubic-bezier(0.16,1,0.3,1)]">
            <div className="flex items-start justify-between gap-4 shrink-0 mb-4 relative">
              ...
              <button onClick={() => setSelectedImage(null)} className="absolute top-0 right-0 p-1 bg-transparent border border-[#A1A1A1] hover:bg-[#A1A1A1]/10 text-zinc-300 hover:text-[#A1A1A1] rounded-md transition-colors shrink-0">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="flex items-start justify-between gap-4 shrink-0 mb-4 relative">
              <div>
                ...
                {selectedGalleryItem.subtitle && (
                  <p className="text-[#A1A1A1] font-mono text-[10px] mt-0.5 uppercase tracking-widest">{selectedGalleryItem.subtitle}</p>
                )}
              </div>
              <button onClick={() => setSelectedGalleryItem(null)} className="absolute top-0 right-0 p-1 bg-transparent border border-[#A1A1A1] hover:bg-[#A1A1A1]/10 text-zinc-300 hover:text-[#A1A1A1] rounded-md transition-colors shrink-0">
                <X size={16} />
              </button>
            </div>
            <div className="flex-1 flex justify-center items-center rounded-xl overflow-hidden bg-[#0e0e10]">
      ...
      <div className="relative z-10">
        {/* Soft Radial Gradient Background Blob for extra depth */}
        <div className="absolute top-[-10%] left-1/2 -translate-x-1/2 w-[800px] h-[600px] bg-[#E6BEA5]/10 blur-[120px] pointer-events-none rounded-full" />
        
        <div className="absolute inset-0 opacity-[0.06] pointer-events-none" style={{ backgroundImage: "linear-gradient(#E6BEA5 1px, transparent 1px), linear-gradient(90deg, #E6BEA5 1px, transparent 1px)", backgroundSize: "60px 60px" }} />
        
        <div className="max-w-6xl mx-auto px-6 pt-32 pb-24 relative z-10">
          <button onClick={onBack} className="group flex items-center gap-2 text-zinc-400 hover:text-[#A1A1A1] transition-colors duration-200 mb-12 font-mono text-xs uppercase tracking-widest">
          ...
          <div className="space-y-4 mb-20 relative">
            <h1 className="font-sans font-extrabold text-5xl lg:text-7xl tracking-tight leading-none text-white">
                Cebu-Bohol <span className="text-[#A1A1A1]">Educational Tour.</span>
            </h1>
              ...
              <div key={index} className="relative pl-12 lg:pl-20">
                ...
                <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 items-start">
                  <div className="w-full lg:w-7/12 flex flex-col gap-6 order-2 lg:order-1">
                    ...
                    <div onClick={() => setSelectedEntry(item)} className="mt-2 bg-[#111113] border border-zinc-800/60 rounded-xl p-5 sm:p-6 cursor-pointer hover:border-zinc-700 transition-colors group shadow-lg relative overflow-hidden">
                      {/* Subtle hover gradient inside the card to match background design */}
                      <div className="absolute inset-0 bg-gradient-to-br from-[#C93C3C]/0 to-[#C93C3C]/0 group-hover:to-[#C93C3C]/5 transition-colors duration-500 pointer-events-none" />
                      
                      <h4 className="font-sans font-bold text-lg text-white mb-3 relative z-10">Journal Entry</h4>
                      <p className="font-sans text-sm text-zinc-400 leading-relaxed line-clamp-4 whitespace-pre-wrap relative z-10">{item.journalEntry}</p>
                      <span className="text-xs font-semibold text-zinc-500 group-hover:text-[#A1A1A1] transition-colors mt-3 inline-block relative z-10">Read full entry &rarr;</span>
                    </div>
                  </div>

                  {item.floatingImage && (
                    <div className="w-full lg:w-5/12 shrink-0 rounded-xl overflow-hidden border-r-[6px] border-[#A1A1A1] shadow-2xl relative cursor-default order-1 lg:order-2 min-h-[240px] bg-zinc-900/40 flex items-center justify-center">
      ...
      <div className="relative w-full overflow-hidden bg-[#070708] py-20 border-t border-zinc-900">
        <div className="max-w-6xl mx-auto px-6 mb-12 relative z-50 text-center">
          <h2 className="font-sans font-bold text-4xl lg:text-5xl tracking-tight text-white drop-shadow-lg">
            Tour <span className="text-[#A1A1A1]">Highlights</span>
          </h2>
        </div>
        ...
        >
          ...
          <div className="absolute pointer-events-none select-none text-center z-0 animate-pulse" style={{ animationDuration: '3s' }}>
            <h3 className="font-sans font-black text-xl md:text-2xl text-white tracking-tight uppercase leading-tight drop-shadow-[0_0_25px_rgba(161,161,161,0.2)]">
              Da best ka <br />
              <span className="text-[#A1A1A1]">Cebu-Bohol</span>
            </h3>
          </div>
            ...
            >
              {globeItems.map((item) => (
                <div
                  ...
                  className="absolute top-0 left-0 w-full h-full rounded-xl overflow-hidden shadow-[0_0_30px_rgba(0,0,0,0.85)] border border-zinc-800/80 pointer-events-auto hover:border-[#A1A1A1] transition-colors bg-zinc-900 group"
                  style={{
                ...
                >
                  ...
                  {item.title && (
                    <div className="absolute inset-x-0 bottom-0 p-2 bg-gradient-to-t from-neutral-950 via-neutral-950/90 to-transparent">
                      <h3 className="text-[#A1A1A1] font-sans text-[10px] font-bold truncate leading-none">{item.title}</h3>
                    </div>
                  )}
                </div>
              ))}
